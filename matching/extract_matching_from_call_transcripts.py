...
def process_json_file(
    ...
    for message in message_list[1:]:  # Skip the first message (usually the opener)
        ...
        conv_path = ConversationalPaths.get(conv_path_id)
        if not conv_path:
            continue
        # conv_path = conv_path
...
def extract_ut_to_conv_path_matching(
    ...
    for assistant_dir in Path(call_transcripts_path).iterdir():
        ...
        for conversation_file in assistant_dir.iterdir():
            call_id = Path(conversation_file.name).stem
            output_dir = Path(
                f"{save_to_path}/matching_dataset/inputs/ut_to_conv_path/{language}/{assistant_id}/{call_id}"
            )
            matching_id = 0
            ...
            process_json_file(conversation_file, assistant_id, call_id, language, output_dir, matching_id)
